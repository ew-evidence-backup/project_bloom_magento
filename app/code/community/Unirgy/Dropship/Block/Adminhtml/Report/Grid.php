    ...
    protected function _prepareCollection()
    {
        if (Mage::helper('udropship')->isSalesFlat()) {
            ...
            $collection->getSelect()
                ->join(array('t'=>$res->getTableName('sales/shipment')), 't.entity_id=main_table.entity_id', array('udropship_vendor', 'udropship_available_at', 'udropship_method', 'udropship_method_description', 'udropship_status', 'shipping_amount', 'subtotal'=>'total_value', 'total_cost'))
                ->join(array('o'=>$res->getTableName('sales/order')), 'o.entity_id=main_table.order_id', array('grand_total', 'order_status'=>'o.status'))
            ...
            $taxAmount = "(select sum(tax_amount) from {$this->t('sales_flat_order_item')} oi inner join {$this->t('sales_order_entity_int')} sio on sio.value=oi.item_id and sio.attribute_id={$sioAttr->getId()} inner join {$this->t('sales_order_entity')} si on si.entity_id=sio.entity_id where si.parent_id=e.entity_id)";
            ...
            $collection = Mage::getResourceModel('sales/order_shipment_collection')
                ...
                ->joinAttribute('grand_total', 'order/grand_total', 'order_id')

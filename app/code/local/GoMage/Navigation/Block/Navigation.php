...
class GoMage_Navigation_Block_Navigation extends Mage_Core_Block_Template
{
    ...
    {        
        $shortCacheId = array(
         ...
         // Add location key to cache ID for browsing categories
         $urlKey = Mage::registry('bloom_nations_zipcode_info');
         if(!($urlKey)) {
            if(isset($_COOKIE['bloom_nations_zipcode'])) {
               $urlKey = $_COOKIE['bloom_nations_zipcode'];
            }
         }
         if('' != $urlKey) {
            $urlKey = strtolower(str_replace(' ','-',str_replace(',','',$urlKey)));
            $shortCacheId['urlkey'] = $urlKey;
         }

...
class GoMage_Navigation_Block_Product_List_Toolbar extends Mage_Catalog_Block_Product_List_Toolbar
{
    ...
    {
        parent::_construct();
        $this->_availableOrder['global_position'] = 'Best Value';
        $this->_availableOrder['price'] = 'Price';
        unset($this->_availableOrder['minimal_vendor_price']);
    ...
    public function getSelectedSort($code = '')
    {
        if(!$this->getRequest()->getParam('order')){
            return ;
        }
        if($code == 'sort_'.$this->getCurrentOrder().'_'.$this->getCurrentDirection())
        {

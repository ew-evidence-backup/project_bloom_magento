                ...
                foreach ($prices as $value) {
                    ...
                    if (isset($options[$attributeId][$value['value_index']])) {
                        ...
                        $productsIndex = array();
										...
										if(!empty($productsIndex))
										{
											$product = $productsIndex[0];
											$vendorId = Mage::registry('vendorId');
											if($vendorId)
											{
												$pv = Mage::getModel('udropship/vendor_product')->getCollection()->addProductFilter($product)->addVendorFilter($vendorId);
												if(count($pv) > 0)
												{
													if($pv->getFirstItem()->getStockQty() > 0)
													{
														$price = Mage::getModel('catalog/product')->load($product)->getVendorPrice(Mage::getModel('udropship/vendor')->load($vendorId));
														$mainPrice = $currentProduct->getVendorPrice(Mage::getModel('udropship/vendor')->load($vendorId));
														
														$info['options'][] = array(
																'id'        => $value['value_index'],
																'label'     => $value['label'],
																'price'     => $price - $mainPrice,
																'oldPrice'  => $price, //$this->_preparePrice($value['pricing_value'], $value['is_percent']),

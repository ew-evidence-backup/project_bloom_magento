...
class Mage_Catalog_Model_Resource_Product_Collection extends Mage_Catalog_Model_Resource_Collection_Abstract
{
    ...
    protected function _construct()
    {
		...
		//$this->addAttributeToFilter('entity_id', 1);
		//$this->addAttributeToSelect('*');
		//$this->addAttributeToFilter('name', array('like', '%adslkd%'));
    }
    ...
    protected function _initSelect()
    {
        ...
        if ($this->isEnabledFlat()) {
            ...
            $this->getSelect()->from(array('e'=>$this->getEntity()->getEntityTable()),array('*'));
        }
        
        // Current Category
        $currentCategory = Mage::registry('current_category');

        //check for Florist (Cities)
        $allowedControllers = array('category', 'product', 'review');
        if(in_array($allowedControllers, Mage::app()->getRequest()->getControllerName())) {
            if($currentCategory) {
                $cidParent = $currentCategory->getParentId();
                $parent_ids = $currentCategory->getPathIds();
            }
        } else {
            $parent_ids = array();
        }
        ...
        $this->getSelect()->joinLeft(array('pz'=>$res->getTableName('custom/productzip')), 'e.entity_id = pz.product_id', array('pz.*'));
        $this->getSelect()->group('e.entity_id');
        // Cookie val
        $cookieVal = Mage::registry('bloom_nations_zipcode');
        $whereZip = false;
        // Little trick to pull 'cookie' from magento registry, so permalinks are possible to work
        if(empty($cookieVal)) {
            $cookieVal = $_COOKIE['bloom_nations_zipcode'];
        } 
        $cookieVal = addslashes($cookieVal);
        if(5 == strlen($cookieVal) ) {
            $whereZip = 'pz.zip = "' .$cookieVal. '"';
        } elseif (5 < strlen($cookieVal)) {
            $whereZip = 'pz.zip IN ('.$cookieVal.')';
        }

        if($whereZip && !Mage::app()->getStore()->isAdmin()) {
            $db = Mage::getSingleton('core/resource')->getConnection('core_read');
            $moduleName = Mage::app()->getRequest()->getModuleName();
            $allowedModules = array('catalog','facebookstore');
            $floristParent = false;
            ...
            if($floristParent) {
                $_SESSION['localBannerBloom'] = false;
                $_SESSION['nolocal'] = false;               
                ...
                if($currentCategory) {
                    ...
                    $query = "
                            ...
                            LEFT JOIN `impl_product_zip` AS `pz` ON e.entity_id = pz.product_id
                            INNER JOIN `catalog_category_product_index` AS `cat_index` ON cat_index.product_id=e.entity_id AND cat_index.store_id=1 AND cat_index.visibility IN(2, 4) AND cat_index.category_id='" . $cid . "'
                            WHERE  (".$whereZip.") GROUP BY `e`.`entity_id`
                            LIMIT 0,1
                    ";
                    ...
                    if(!$counted){
                        $this->getSelect()->where('pz.zip = 999999');
                        $_SESSION['localBannerBloom'] = true;
                        $_SESSION['nolocal'] = true;
                    } else {
                        $_SESSION['localBannerBloom'] = false;
                        $_SESSION['nolocal'] = false;
    
                        $this->getSelect()->where($whereZip);
                    }
                }
            }
        }
    ...
    public function addWebsiteNamesToResult()
    {
        ...
        if (!empty($productWebsites)) {
            $select = $this->getConnection()->select()
                ->from(array('product_website' => $this->_productWebsiteTable),array('*'))
                ->join(

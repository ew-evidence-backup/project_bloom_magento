        ...
        if ($categoriesData) {
            ...
            foreach ($categoriesData as $delSku => $categories) {
                ...
                foreach (array_keys($categories) as $categoryId) {
                    $categoriesIn[] = array('product_id' => $productId, 'category_id' => $categoryId, 'position' => $this->_newSku[$delSku]['position']);
                }
            }
            ...
            if ($categoriesIn) {
				Mage::log("Position: ",null,'pzip.log');
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            $this->_saveProductEntity($entityRowsIn, $entityRowsUp)
                ...
                ->_saveProductAttributes($attributes);
						...
						{
							foreach($entityRowsIn as $val)
							{
								if(isset($val['sku']) && !empty($val['sku']))
								{
									
									$product = Mage::getModel('catalog/product')->getCollection()->addAttributeToSelect('*')->addAttributeToFilter('sku', $val['sku'])->getFirstItem();
									
									if($product->getId())
									{
										Mage::log($product->getId(), null, 'mylogfile.log');
										$defaultVendorId = Mage::helper('udropship')->getLocalVendorId($product->getStoreId());
										$product->storeVendorData($defaultVendorId);

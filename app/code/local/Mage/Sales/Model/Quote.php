...
class Mage_Sales_Model_Quote extends Mage_Core_Model_Abstract
{
    ...
    public function addProductAdvanced(Mage_Catalog_Model_Product $product, $request = null, $processMode = null)
    {
        ...
        foreach ($cartCandidates as $candidate) {
            ...
            if(!$vendorId = $request->getParam('v', false)) {
                $vendorId = Mage::registry('current_svendor');
            }
            if(!$vendorId)
            {
                Mage::app()->getFrontController()->getResponse()->setRedirect('/');
            }
            $candidate->setUdropshipVendor($vendorId);						
    ...
    protected function _addCatalogProduct(Mage_Catalog_Model_Product $product, $qty = 1)
    {
        ...
        $item = $this->getItemByProduct($product);
			...
			//Mage::log("Product " . $product->getId() . '-' . $item->getUdropshipVendor(), null, "your_log_file.log");					
        if (!$item || ($item && $item->getUdropshipVendor() != $product->getUdropshipVendor())) {
    ...
    {
        foreach ($this->getAllItems() as $item) {
            //if ($item->getProductId() == $product->getId() && $item->getUdropshipVendor() == $product->getUdropshipVendor()) {
            //    return $item;
            //}

							if($item->representProduct($product) && $item->getUdropshipVendor() == $product->getUdropshipVendor())
							{
								return $item;
							}
        }

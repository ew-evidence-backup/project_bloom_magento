...
class Web_QuickCheckout_OnestepController extends Mage_Checkout_OnepageController
{
    ...
    public function saveBillingAction()
    {
        ...
        if ($data) {
            ...
            $this->getResponse()->setBody(Zend_Json::encode($result));
            Mage::dispatchEvent("checkout_save_billing_after", array('request' => $this->getRequest()));
        }
    }
    ...
    public function saveShippingAction()
    {
        ...
        $this->getResponse()->setBody(Zend_Json::encode($result));
        Mage::dispatchEvent("checkout_save_shipping_after", array('request' => $this->getRequest()));
    }
    ...
    public function saveOrderAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                Mage::log($result, 0, 'transaction.log');
                $this->getResponse()->setBody(Zend_Json::encode($result));
                return;
            }
            catch (Mage_Core_Exception $e) {
                ...
                $result['success'] = false;
                $result['error'] = true;
                $result['error_messages'] = 'Exception: ' . $this->__('Unable to set Payment Method. ') . $e->getMessage();
                Mage::log($result, 0, 'transaction.log');
                $this->getResponse()->setBody(Zend_Json::encode($result));
                return;
            }
